...
const UserSchema = z.object({
  ...
  lastSeen: z.date().transform((val) => new Date(val)).optional(), // optional, but should be a valid timestamp
  rating: z.number().optional(), // default to 0
  accountType: z.enum(['personal', 'business']).default('personal'), // allowed account types
  createdAt: z.date().transform((val) => new Date(val)).default(() => new Date()), // default to current timestamp
