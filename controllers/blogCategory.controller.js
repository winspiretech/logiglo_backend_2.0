...
const createBlogCategory = async (req, res) => {
  try {
    ...
    const formattedName =
      blogCategory.trim().charAt(0).toUpperCase() +
    ...
    if (prevData) {
      throw new ApiError(
        400,
        'The blog category alredy exists',
        'Blog category exists',
      );
    }
    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          allCategories,
          'Blog categories fetched succcessfully',
        ),
      );
  } catch (error) {
    if (error instanceof ApiError) {
  ...
  try {
    ...
    if (!updatedValue) {
      throw new ApiError(
        500,
        'Internal server error',
        'Something went wrong while updating value',
      );
    }
    res
      .status(200)
      .json(
        new ApiResponse(200, updatedValue, 'Category updated successfully'),
      );
  } catch (error) {
    if (error instanceof ApiError) {
      return res.status(error.statusCode).json(error);
